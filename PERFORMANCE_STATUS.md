...
- Готовность к commit/push: **PUBLISHED** — откат defer CSS: feature `e529bca9`, plesk `822f1cbf` (возврат к состоянию ~89 mobile)
...
| После этапов 3–9 | ПК | 62 | 0,5 с | 0,8 с | 530 мс | 0,262 | 1,6 с |
| После этапов 3–9 | Мобильный | **89** | **1,8 с** | **3,4 с** | **90 мс** | **0,004** | 3,8 с |
| После defer CSS (регрессия) | Мобильный | 61 | 3,8 с | 7,7 с | 230 мс | 0,001 | 4,9 с |

> Лучший mobile baseline — **89**. Deferred `public.bundle.css` обвалил LCP до 7,7 с → откат на проде. Не повторять defer Mottor CSS без полного shell.
